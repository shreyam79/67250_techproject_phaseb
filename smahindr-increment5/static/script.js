...
function updateMobileSummary() {
    const prices = { general: 18, student: 10, member: 14, under18: 0 };
    const types = ['general', 'student', 'member', 'under18'];
    types.forEach(function(type) {
        const qty = parseInt(document.getElementById('qty-' + type).innerText) || 0;
        const price = prices[type];
        const subtotal = qty * price;
        const el = document.getElementById('mobile-summary-' + type);
        if (el) el.innerText = qty + 'x$' + price + ' = $' + subtotal;
    });
}
